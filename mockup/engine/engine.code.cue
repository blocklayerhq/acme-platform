		...
		for actionName, _ in Component.action {
			...
			"\(actionName)": {
				...
				script: {
					...
					buildx: "docker-buildx build --progress=auto \(cacheTo)" 
					componentCacheFlags=strings.Join([action.component[e.name][c.name][actionName].cacheFlag for _, c in e.component], " ")
				}
				...
				dockerfile: """
					...
					# Run '\(actionName)' entrypoint of each bot
					# FIXME: connect component dependency graph with 'COPY --from='
					\(strings.Join([
						action.component[e.name][c.name][actionName].dockerfile
	...
	for _, c in e.component {
		...
		engine action component "\(e.name)" "\(c.name)":  {
			...
			<actionName>: {
				...
				cacheFlag: *""|string
				if !cache {
					cacheFlag: "--build-arg \(cacheKey)=$(date +%s)-$RANDOM"
				}
				...
				dockerfile: """
					...
					ARG \(cacheKey)=42
					# Run the '\(actionName)' entrypoint of component '\(c.name)' in env '\(e.name)'
					RUN --mount=type=cache,target=/workspace/cache /entrypoint.sh \(actionName)
					"""
			}
		}
	}
